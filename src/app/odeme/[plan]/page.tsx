...
export default function OdemePage({ params }: { params: { plan: string } }) {
  ...
  // Plan bulunamadıysa ana sayfaya yönlendir
  useEffect(() => {
    if (!plan) {
  ...
  // Enterprise planı için farklı sayfa göster (iletişim formu)
  ...
  // Fiyat hesaplama (yıllık ödeme için %15 indirim)
  const calculatePrice = () => {
    const basePrice = parseInt(plan.price);
    if (billingCycle === "yearly") {
      return (basePrice * 12 * 0.85).toFixed(0); // %15 indirim
    }
    return basePrice;
  };
